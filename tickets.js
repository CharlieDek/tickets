...
function applyAllFilters() {
    ...
    const hasEmptyFilter = 
        ...
        filterState.hour_of_day.size === 0 ||
    ...
    filters.push(['in', ['to-string', ['get', 'hour_of_day']], ['literal', Array.from(filterState.hour_of_day).map(String)]]);
...
function setupFilterListeners() {
    document.querySelectorAll('input[name="hour_of_day"]').forEach(cb => {
        cb.addEventListener('change', () => {
            updateFilter('hour_of_day', cb.value, cb.checked);
        });
    });
    ...
    document.getElementById('hour-toggle-all')?.addEventListener('click', () => {
        const hourBoxes = Array.from(document.querySelectorAll('input[name="hour_of_day"]'));
        const allChecked = hourBoxes.every(cb => cb.checked);
        hourBoxes.forEach(cb => {
            cb.checked = !allChecked;
            updateFilter('hour_of_day', cb.value, cb.checked);
        });
        document.getElementById('hour-toggle-all').textContent = allChecked ? 'Select all' : 'Deselect all';
    });
